...
class CharRNN:
    def __init__(self, num_classes, num_seqs=64, num_steps=50,
        ...
        self.saver = tf.train.Saver() # 用来保存模型
    ...
    def build_lstm(self):
        ...
        with tf.name_scope('lstm'):
            ...
            # 通过lstm_outputs得到概率
            seq_output = tf.concat(self.lstm_outputs, 1) # 拼接张量
            x = tf.reshape(seq_output, [-1, self.lstm_size]) # -1代表自动计算所需维度
    ...
    def build_optimizer(self):
        ...
        self.optimizer = train_op.apply_gradients(list(zip(grads, tvars)))
    ...
    def train(self, batch_generator, max_steps, save_path, save_every_n, log_every_n):
        ...
        with self.session as sess:
            ...
            for x, y in batch_generator:
                ...
                if step >= max_steps: # 超过最大迭代次数，终止训练
                    break
            self.saver.save(sess, os.path.join(save_path, 'model'), global_step=step)
